...
export class ViewerEffects {
  ...
  init$ = createEffect(
    () => this.actions$.pipe(
      ...
      tap(() => {
        ...
        this.store.dispatch(ViewerActions.setLegacy({
          payload: {
        ...
        navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' } })
          .then((mediaStream) => {
            ...
            this.videoService.bufferSource.addEventListener('sourceopen', () => {
              ...
              this.videoService.mediaRecorder.ondataavailable = (e) => {
                ...
                this.store.dispatch(ViewerActions.setLegacy({ payload: { lastReceived: this.lastReceived } }));
              ...
              interval(1000).pipe(
                ...
                takeWhile(isEnded => !isEnded)).subscribe();
              this.isInitialized = true;
              this.store.dispatch(ViewerActions.setLegacy({ payload: { isInitialized: this.isInitialized } }));
            });
            this.isLive = true;
          ...
          .catch((e) => {
            if (e.name === 'PermissionDeniedError' ||  // Chrome
              ...
              this.store.dispatch(ViewerActions.setLegacy({ payload: { isPermissionDeniedError: this.isPermissionDeniedError } }));
            } else if (e.name === 'NotFoundError') {
              this.isNotFoundError = true;
              this.store.dispatch(ViewerActions.setLegacy({ payload: { isNotFoundError: this.isNotFoundError } }));
            } else {
              this.isUnknownError = true;
              this.store.dispatch(ViewerActions.setLegacy({ payload: { isUnknownError: this.isUnknownError } }));
      ...
      switchMap((action) => {
        switch (action.type) {
          ...
          case ViewerActions.stopRecord.type:
            return this.changeDelay(0, true).pipe(
    ...
    this.actions$.pipe(
      ...
      tap(() => {
        if (!this.isLive) {
          ...
          this.waitTime = 0;
          this.isLive = true;
          this.store.dispatch(ViewerActions.setLegacy({ payload: { waitTime: this.waitTime, isLive: this.isLive } }));
        }
      })
    ),
    { dispatch: false }
  ...
  play$ = createEffect(() =>
    this.actions$.pipe(
      ...
      tap(() => {
        ...
        this.waitTime = 0;
        this.isStopped = false;
        this.store.dispatch(ViewerActions.setLegacy({ payload: { waitTime: this.waitTime, isStopped: this.isStopped } }));
      })
    ),
    { dispatch: false }
  );
  ...
  private changeDelay(ms: number, noWait = false): Observable<Action> {
    ...
    if (headroom < 0) {
      ...
      return concat(
        ...
        timer((-headroom) % 1000, 1000).pipe(
          takeUntil(this.store.pipe(
            ...
            filter(x => x))),
          take(periods),
          switchMap((i: number): Observable<Action> => {
            if (i < periods - 1) {
              return from([
      ...
      const actions: Action[] = [ViewerActions.setTime({
